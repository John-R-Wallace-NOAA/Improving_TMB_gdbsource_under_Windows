...
# The pwd path issue always puts the current working directory as a prefix on the path of 'Rterm' even when using 'unset environment':
#     Error creating process /c/TMB_Debug/C:\R\R-42~1.1\bin\x64\Rterm.exe, (error 2).  
# The only way this worked for me, without the fix below, is to remove Sys.which() from 'Rterm' and manually set the working 
# directory in R to Sys.which('Rterm') [e.g. C:/R/R-4.2.1/bin/x64].

# Note that on the GDB command line, the pwd path is set with 'cd' but looked with 'pwd'.
